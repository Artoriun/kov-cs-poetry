...
- **Custom Slides** button on each poem card (between Save and Feature) — opens an editable slide panel pre-split from the poem's overlay text
  ...
  - Pressing **Custom Slides** again (labelled "Original" when active) immediately reverts the poem-detail to its auto-split mode without requiring a Save — `customSlidesEnabled` is written to Firestore instantly; the slide content is preserved so it reappears on next open
